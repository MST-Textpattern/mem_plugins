...
p(tag-summary). This tag will output an HTML form. The HTML form field names must follow the pattern "@<type>_<name>@", where @<type>@ is a value from 
	the below type list. The database table's field name must match @<name>@ exactly.
...
function mem_simple_form($atts, $thing='')
{
    ...
    if (empty($atts['table']))
		...
		foreach(array('table','id_field','id_insert','ignore_fields') as $a) {
...
function mem_simple_form_submitted()
{
  ...
  if (empty($table))
  {
	...
	$ignore_fields = split(',', $ignore_fields);
	...
	foreach($mem_form_values as $k=>$v)
	{
			...
			if (strncmp($k, 'mem_simple_', 11) == 0 || array_search($k, $ignore_fields) === FALSE)
				continue;
